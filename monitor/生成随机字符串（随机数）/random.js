window.random = (function(){
    ...
    function random() {
        var chars = {
            ...
            symbol: '~!@#$%^&{}(){}<>+-*/=?_`|:;",.',
            fcode: '０１２３４５６７８９ａｂｃｄｅｆｇｈｉｊｋｌｍｎｏｐｑｒｓｔｕｖｗｘｙｚＡＢＣＤＥＦＧＨＩＪＫＬＭＮＯＰＱＲＳＴＵＶＷＸＹＺ',
            fnumber: '０１２３４５６７８９',
            flower: 'ａｂｃｄｅｆｇｈｉｊｋｌｍｎｏｐｑｒｓｔｕｖｗｘｙｚ',
            fupper: 'ＡＢＣＤＥＦＧＨＩＪＫＬＭＮＯＰＱＲＳＴＵＶＷＸＹＺ',
            fsymbol: '～！＠＃＄％＾＆［］（）｛｝＜＞＋－＊／＝？＿｀＼｜：；＂＇，．'
        };
        // 汉字
        this.chs = function chs(len) {
            return this.create(chars.chs, len);
        };
        /**
         * 半角数字加字母
         * @len int 生成字符的长度
         * @indi boolean 是否去除模糊字符
         * @return string
         */
        this.code = function code(len, indi = true) {
            var cs = true == indi ? chars.code.replace(/[0oO1Il]/g, '') : chars.code;
            return this.create(cs, len);
        };
        // 全角数字加字母
        this.fcode = function fcode(len, indi = true) {
            var cs = true == indi ? chars.fcode.replace(/[０ｏＯ１Ｉｌ]/g, '') : chars.fcode;
            return this.create(cs, len);
        };
        /**
         * 半角数字
         * 调用：生成数字位数为len的随机数
         *  number(number len = 1)
         * 调用：生成 min <= num <= max 区间的随机整数或保留n位小数的浮点数
         *  number(number min, number max[, boolean isInt = true[, number decimal_place = 2]])
         */
        this.number = function number() {
            ...
            // 生成某个区间的数字
                ...
                // 是否返回整形
                var isInt = false == arguments[2] ? false : true;
                if (isInt) {
                    min = Math.floor(min);
                    max = Math.floor(max);
                ...
                // 保留小数位数
                var dec = Math.pow(10, (arguments[3] && /^[1-9]\d*$/g.test(arguments[3].toString())) ? arguments[3] : 2);
                return Math.round((Math.random() * (max - min) + min) * dec) / dec;
            }
        };
        // 全角数字
        this.fnumber = function fnumber(len) {
            return this.create(chars.fnumber, len);
        };
        // 半角小写字母
        this.lower = function lower(len) {
            return this.create(chars.lower, len);
        };
        // 全角小写字母
        this.flower = function flower(len) {
            return this.create(chars.flower, len);
        };
        // 半角大写字母
        this.upper = function upper(len) {
            return this.create(chars.upper, len);
        };
        // 全角大写字母
        this.fupper = function fupper(len) {
            return this.create(chars.fupper, len);
        };
        // 半角标点
        this.symbol = function symbol(len) {
            return this.create(chars.symbol, len);
        };
        // 全角标点
        this.fsymbol = function fsymbol(len) {
            return this.create(chars.fsymbol, len);
        };
        // 自定义
        this.create = function create(chars, len = 1) {
            if (typeof chars != 'string' || isNaN(len) || len < 1) {
                return '';
            }
            var temp = [];
            var nots = true;
